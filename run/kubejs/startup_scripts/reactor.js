...
GTCEuStartupEvents.registry('gtceu:machine', event => {
    ...
    $GNTMachineUtils.registerFissionReactor(
        "small_fission_reactor", "Small Fission Reactor", GTValues.LuV, 1,
    ...
    $GNTMachineUtils.registerFissionReactor(
        "large_fission_reactor", "Large Fission Reactor", GTValues.UV, 4,
        GNTBlocks.RADIATION_PROOF_CASING, GTBlocks.CASING_TITANIUM_TURBINE,
        definition => FactoryBlockPattern.start()
            .aisle('#####CCCCC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CCCCC##')
            .aisle('####CCCCCCC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CCCCCCC#')
            .aisle('TTTCCCCCCCCC', 'TDTCMMMMMMMC', 'TTTCMMMMMMMC', '###CMMMMMMMC', '###CMMMMMMMC', '###CMMMMMMMC', '###CCCCCCCCC')
            .aisle('TTTWCCCCCCCC', 'TGTPPMRMRMMM', 'TTTMMMRMRMMM', '###MMMRMRMMM', '###MMMRMRMMM', '###MMMRMRMMM', '###CCCCCCCCC')
            .aisle('TTTCCCCCCCCC', 'TGTMMMMMMMMM', 'TTTMMMMMMMMM', '###MMMMMMMMM', '###MMMMMMMMM', '###MMMMMMMMM', '###CCCCCCCCC')
            .aisle('TTTWCCCCCCCC', 'TGTPPMRMRMMM', 'TTTMMMRMRMMM', '###MMMRMRMMM', '###MMMRMRMMM', '###MMMRMRMMM', '###CCCCCCCCC')
            .aisle('TTTCCCCCCCCC', 'TDTCMMMMMMMC', 'TTTCMMMMMMMC', '###CMMMMMMMC', '###CMMMMMMMC', '###CMMMMMMMC', '###CCCCCCCCC')
            .aisle('####CCCCCCC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CMMMMMC#', '####CCCCCCC#')
            .aisle('#####CC@CC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CMMMC##', '#####CCCCC##')
            .where("@", Predicates.controller(Predicates.blocks(definition.get())))
            .where("C", Predicates.blocks(GNTBlocks.RADIATION_PROOF_CASING.get()).setMinGlobalLimited(50)
                .or(Predicates.autoAbilities(definition.getRecipeTypes()))
                .or(Predicates.abilities(PartAbility.MAINTENANCE).setExactLimit(1)))
